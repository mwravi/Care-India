...
class BeneficiaryQuizActivity : BaseActivity() {
    ...
    var question: ArrayList<MstFormQuestionModel?>? = null
    ...
    override fun initializeController() {
        ...
        btn_save.setOnClickListener {
            ...
            if (checkValidation == 100 || checkValidation == 101) {
                validate!!.CustomAlert(this@BeneficiaryQuizActivity, "  $alertMsg ")
            } else {
                SaveRecord()
...
//        finish()
    ...
    fun fillQuestion() {
        ...
        questionPageNo = mstAssessmentViewModel.getDistinctPageBen(2)
        ...
        if (questionPageNo != null) {
            for (i in currentpage until questionPageNo!!.size) {
                ...
                questionPageNo = mstAssessmentViewModel.getDistinctPageBen(2)
                idformName.text =
                    mstAssessmentViewModel.getPageHeader(questionPageNo!![i].PageID!!, 2)
                ...
                question = dataProvider.getMSTFormQuestion(
                    2,
                    iLanguageID,
                    questionPageNo!![i].PageID!!,
                    validate!!
                ...
                if (question != null && question!!.size > 0) {
                    ...
                    for (j in question!!.indices) {
                        ...
                        val data = beneficiaryDetailViewModel.getData(
                            Ben_GUID,
                        ...
                        tvDependentQuestion!!.setText(java.lang.String.valueOf(question!![j]!!.DependentQuestion))
                        tvDependentQuestion!!.setVisibility(View.GONE)
                        linear!!.addView(tvDependentQuestion)

                        tvDependentOption = TextView(this)
                        tvDependentOption!!.setText(java.lang.String.valueOf(question!![j]!!.DependentOption))
                        tvDependentOption!!.setVisibility(View.GONE)
                        ...
                        if (question!![j]!!.QuestionType == 1) {
                            ...
                            if (data.size > 0) {
                                edit_text_type.setText(data.get(0).QuestionValue!!)

                            }

                            if (question!![j]!!.DependentQuestion!!.isNotEmpty() && data.size > 0 && data.get(
                                    0
                                ).QuestionValue!!.isNotEmpty()
                            ) {
                                tbl_row_edit.visibility = View.VISIBLE
                                tblrow1.visibility = View.VISIBLE
                            } else if (question!![j]!!.DependentQuestion!!.isNotEmpty()) {
                                tbl_row_edit.visibility = View.GONE
                            ...
                            var ID = 0
                            if (data.size > 0) {
                                ID = validate!!.returnIntegerValue(data.get(0).QuestionValue!!)
                            ...
                            edit_text_type.setInputType(InputType.TYPE_CLASS_NUMBER or InputType.TYPE_NUMBER_FLAG_DECIMAL)
                            if (data.size > 0) {
                                edit_text_type.setText(data.get(0).QuestionValue!!)
        ...
        for (j in 0 until iSize) {
            ...
            if (ll.visibility == View.VISIBLE) {
                if (ll.childCount > 0) {
                    ...
                    val count = beneficiaryDetailViewModel.getAllCount(
                        validate!!.RetriveSharepreferenceString(AppSP.Ben_GUID)!!,
                        QusID
                    )
                    if (count > 0) {
                        beneficiaryDetailViewModel.updateData(
                            validate!!.RetriveSharepreferenceString(AppSP.Ben_GUID)!!,
                            QusID, QusAns
                        )
                    } else {
                        beneficiaryDetailViewModel.insert(
                            BeneficiaryDetailEntity(
                                validate!!.RetriveSharepreferenceString(AppSP.Ben_GUID)!!,
                                QusID,
                                QusAns,
                                1
